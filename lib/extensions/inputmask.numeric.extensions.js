    ...
    preValidation: function (
      ...
      if (
        ...
        !maskset.validPositions.some(
          vp => vp && vp.generatedInput !== true && vp.match.generated !== true
        )
      ) {
        // prepend "0" when typing the radix on a field with no integer digit
        // (e.g. "." -> "0."). Fixes #2561.
        const intPos = findValidator.call(inputmask, "9", maskset);
        return {
          insert: [{ pos: intPos, c: "0", fromIsValid: true }],
          rewritePosition: intPos + 1
        };
      }
      if (
        radixPos !== -1 &&
        ...
        switch (e.type) {
          ...
          case "checkval":
            ...
            if (buffer[buffer.length - 1] === opts.negationSymbol.front) {
              ...
              } else if (radixNDX > opts.suffix.length) {
                // prepend "0" on blur when a fractional digit is present but
                // no integer digit precedes the radix (e.g. ".5" -> "0.5").
                // Fixes #2561.
                const digit = /[\d\uFF10-\uFF19\u0660-\u0669\u06F0-\u06F9]/;
                const hasFractionalDigit = buffer
                  .slice(opts.suffix.length, radixNDX)
                  .some(c => digit.test(c));
                const precedingChar = buffer[radixNDX + 1];
                if (
                  hasFractionalDigit &&
                  (precedingChar === undefined || !digit.test(precedingChar))
                ) {
                  buffer.splice(radixNDX + 1, 0, "0");
                  result = {
                    refreshFromBuffer: true,
                    buffer: stripBuffer(buffer)
                  };
